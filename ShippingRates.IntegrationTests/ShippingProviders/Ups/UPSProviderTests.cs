...
[TestFixture, Ignore("Invalid test credentials")]
public class UPSProviderTests
{
    ...
    public void OneTimeSetUp()
    {
        ...
        UPSAccountNumber = config.UPSAccountNumber ?? throw new ArgumentException(nameof(config.UPSAccountNumber));
        UPSClientId = config.UPSClientId ?? throw new ArgumentException(nameof(config.UPSClientId));
        UPSClientSecret = config.UPSClientSecret ?? throw new ArgumentException(nameof(config.UPSClientSecret));
    }
    ...
    [Test, Ignore("No longer works in sandbox")]
    public async Task UPSCurrency()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(r, Is.Not.Null);
            Assert.That(fedExRates, Is.Not.Empty);
            Assert.That(fedExRates.Any(r => r.CurrencyCode != "EUR"), Is.True);
        }
    }
