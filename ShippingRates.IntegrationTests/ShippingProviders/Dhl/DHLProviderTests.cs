...
public class DHLProviderTests
{
    ...
    [Test, Ignore("No longer works with sandbox")]
    public void DHLIncludeServices()
    {
        var from = new Address("", "", "75003", "FR");
        var to = new Address("", "", "75011", "FR");
        var package = new Package(7, 7, 7, 6, 0);

        var configuration = GetConfiguration().IncludeServices(['I']);

        var rates = GetRateManager(configuration).GetRates(from, to, package);
        Assert.That(rates, Is.Not.Null);
        Assert.That(rates.Rates, Is.Not.Empty);
        Assert.That(rates.Rates, Has.Some.Matches<Rate>(r => r.Name.Contains("DOMESTIC 9:00")));
    }

    [Test]
    public void DHLExcludeServices()
    {
        var from = new Address("", "", "75003", "FR");
        var to = new Address("", "", "75011", "FR");
        var package = new Package(7, 7, 7, 6, 0);

        var configuration = GetConfiguration().ExcludeServices(['C']);

        var rates = GetRateManager(configuration).GetRates(from, to, package);
        Assert.That(rates, Is.Not.Null);
        Assert.That(rates.Rates, Is.Not.Empty);
        Assert.That(rates.Rates, Has.None.Matches<Rate>(r => r.Name.Contains("MEDICAL")));
    }
    ...
    public async Task DHLSaturdayDelivery()
    {
        ...
        Assert.That(dhlRates, Has.Some.Matches<Rate>(r => r.Options.SaturdayDelivery));
    }
