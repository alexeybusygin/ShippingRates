USPS Special Services can be requested using `UspsProviderConfiguration.ExtraServiceCodes` field:
...
```CSHARP
    ...
    var configuration = new UspsProviderConfiguration()
    {
        ...
        MailClasses = [UspsMailClass.LibraryMail],
        ExtraServiceCodes = [UspsExtraServiceCode.RegisteredMail, ...]
    };
    rateManager.AddProvider(new UspsProvider(configuration));
```

Please note that when extra service codes are included, it is recommended to specify `MailClasses` explicitly.

### Automatic Extra Services

The following special services are applied automatically when applicable:

1. If any package has `SignatureRequiredOnDelivery` set, the provider will automatically include `UspsExtraServiceCode.AdultSignatureRequired` and `UspsExtraServiceCode.CertifiedMailAdultSignatureRequired`.

2. If any package specifies an `InsuredValue`, the provider will automatically apply either `UspsExtraServiceCode.InsuranceUpTo500` or `UspsExtraServiceCode.InsuranceOver500`, depending on the amount.
